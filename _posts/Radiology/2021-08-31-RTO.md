...
八月外訓，選擇去放射腫瘤科（radiation oncology）。每天的行程表如下：

|     | Morning Meeting | AM              | PM             |
| :-- | :-------------- | :-------------- | :------------- |
| W1  |                 | Hybrid-OP room  | Hybrid-OP room |
| W2  |                 | Brachytherapy   | Simulation     |
| W3  | Chart Round     |                 | OPD shadowing  |
| W4  |                 |                 | RTO room       |
| W5  |                 | Hybroid-OP room | Hybrid-OP room |

週一週五如果沒有 day off 就去跟我們科在刀房內的血管攝影室。週三早上有放腫住院醫師的病例報告與主治醫師評論治療計畫的晨會。其他天就參觀各種模擬攝影、近接還有門診。

## 放腫科工作流程之觀察

一個新病人通常不會直接去放腫科，通常是已經確診了某個類型的癌症，同時在腫瘤科（內科、oncology）或外科有看診這樣。大部分的流程似乎入下面所示：
...
- 醫師：醫師的工作主要是門診（我觀察下來似乎佔據 3/5 的工作量）。其他就是本業 planning，還有參加 combine meeting。年輕主治醫師還需要留下來留守小夜的檢查。
- 物理師：在放腫科另外有一批神秘的物理師。我一開始以為是升級版的放射師（類似護理師轉職專科護理師、麻醉護理師之類的），後來發現不是。物理師算是有額外進修的放射師，具備醫學物理的學位（碩博士），工作內容是**病患實際治療計畫的模擬與機器參數設計**，以及管理輻射暴露相關的流程。
- 放射師：實際操作機器、執行治療計畫。
- 護理師：on IV、抽血、衛教。

## Mold Room & Simulation 模擬攝影

病患初診確定治療計畫後就會去模具室（mold room）打造符合身形的模板（cast、泡綿）。因為放射腫瘤治療（radiotherapy）特別講求姿勢的一致性，來確定腫瘤部分有接收到足夠的輻射暴露，同時降低周圍組織的傷害。模具做好後就會去做模擬攝影。

模擬攝影的目的是建立一個與治療時一致的擺位，來決定機器照射的範圍。不過，與我們科的機器相比，放腫科的 CT 有一些差別：
...
- 近接 = brachytherapy 以及 intraBeam

質子治療算是我們醫院的大特色之一。質子束的 Bragg peak 在減速到一個程度會突然釋放，因此輻射劑量在預計部位以外很少，所以副作用較 conventional radiotherapy 少很多，缺點是全自費，一個療程快一百萬。
...
- 認識放腫科的主流期刊：Red Jounral（International Jounral of Radiation Oncology, Biology and Physics / ASTRO 美國放射腫瘤學會）以及 Green Journal（ Radiotherapy & Oncology / ESTRO 歐洲放射腫瘤學會）
