---
title: Fluent Python Chap 01 - Data Model
hidden: true
layout: post
category: Python
---
...
- 標準物件例如 list，其 special methoid 在 Cython 這類實現中，會直接去存取 C 結構內的欄位，速度非常快。
- 不要自己建立奇怪的 special methoid 例如什麼 ``__foo__``，因爲難保將來被使用到。

## Numeric types - respond to operators
...
- ``__bool__`` 提供給 ``bool()`` 這個函數呼叫：
    ...
    - 在 The Python Standard Libarary - Built-in Types 中，定義了物件 Boolean 相關的測試。
