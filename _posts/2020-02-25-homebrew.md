...
[Homebrew](https://brew.sh/index_zh-tw) 是一個「套件管理工具」。在 macOS 以及 Linux 這種 Unix-like 的作業系統中，系統級的軟體會安裝在諸如 ``/bin`` 以及 ``/usr/bin`` 等資料夾內（參考阮一峰的文章 [Unix 目录结构的来历](https://www.ruanyifeng.com/blog/2012/02/a_history_of_unix_directory_structure.html)，裏面解釋了目錄設計）。
