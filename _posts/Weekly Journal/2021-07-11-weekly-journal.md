...
買了 M1 iPad Pro 之後，就想試試能不能在上面跑程式、做點雜活。不過，試玩了一下略覺失望。一個是虛擬機軟體 UTM，一個是 Alpine Linux 的包裝叫 iSH。UTM 據說可以執行 Windows ARM 之類的軟體，不過實際上因為系統限制的原因，記憶體最多只能使用到 40%，而且性能限制在單核心。我改跑了 Arch Linux ARM，一直有各種內核錯誤。iSH 運行起來流暢很多，但是沒有辦法安裝不少理論上有提供 ARM 版本的軟體（例如 R、Racket）。Python 雖然可以安裝跟運行，但是無法安裝常見的套件（例如 Pandas）。結論是：不要浪費時間玩這些。
